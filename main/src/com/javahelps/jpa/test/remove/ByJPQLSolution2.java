...
public class ByJPQLSolution2 {
    public static void main(String[] args) {
        ...
        {//уйти от дополнительного запроса на выборку сущности можно путем полной очистки котекста персистентности. но в таком случае мы теряем некую часть оптимизции
            entityManager.getTransaction().begin();

            entityManager.createQuery("DELETE FROM "+User.class.getName()+" u WHERE u.id=:id").setParameter("id", 1L).executeUpdate();
            entityManager.clear();

            entityManager.getTransaction().commit();
        }

        {//хибернейт загрузит данные из базы, т.к. контекст больше не будет содержать удаленную сущность
            entityManager.getTransaction().begin();
            System.out.println();
